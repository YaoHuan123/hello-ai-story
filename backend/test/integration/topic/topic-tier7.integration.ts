...
async function main(): Promise<void> {
  ...
  check("tier7.json 已生成", fs.existsSync(tierPendingPath(TEST_USER, 7)));
  check("返回数组", Array.isArray(picks), picks);
  check("tier pending.tier === 7", readTierPending(TEST_USER, 7)?.tier === 7);
  ...
  console.log("\n=== advance 7→8 ===");
  check("进入 tier8", advanceStage(TEST_USER).tier === 8);
