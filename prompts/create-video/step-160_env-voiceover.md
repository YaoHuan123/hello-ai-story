...
2. **No shots (count 0)**: that row may have multiple short lines; each still ≤ **80 characters**.
...
4. **Source switch transition**: if `adjacencyHints` marks `switchFromPrev=true` (era ↔ personal switch), the first line of that `voiceover` must include a **first-person bridge** (e.g. "Back then I…", "Against that backdrop, I…") with at least one time/place/situation/event anchor; no empty sentiment.
...
7. **Main line — information focus**: the full `voiceover` array must recover **clear time and place** (may concentrate in first one or two lines; not every line needs full name/year); event summary spread across lines; avoid shot-by-shot repetition. With shots: **each line ≤ 80 characters**.
8. **Era layer — style**: first-person bridges OK for continuity with personal segments; do not invent new "I was there" facts; with shots `voiceover.length === eraVisualSceneCounts[j]`; without shots each line ≤ 80 characters; no invented policy names or statistics.
...
From **`PIPELINE_JSON`**, generate voiceover. Output **only one JSON object**: **no** Markdown fences, no preamble, do not repeat hard rules above.
